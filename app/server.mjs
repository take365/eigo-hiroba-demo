...
async function pronunciation(req, expected) {
  ...
  return {transcript,matched,feedback:matched?`「${expected}」と聞こえたよ！すごいね 🎉`:`おしい！「${expected}」を、もう一度ゆっくり言ってみよう。`};
}
const server = createServer(async (req,res) => {
  try {
    if (req.method === 'OPTIONS') return send(res,204,'');
    if (req.method === 'POST' && req.url === '/api/speak') { const {text} = await json(req); const data=await speak(text); return send(res,200,data,{'Content-Type':'audio/mpeg'}); }
