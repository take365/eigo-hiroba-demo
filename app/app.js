...
let cardIndex = 0, quizIndex = 0, speakIndex = 0, pairIndex = 0, quizCorrect = new Set(), speakCorrect = new Set(), pairCorrect = new Set();
...
function showView(id) { document.querySelectorAll('.view').forEach(v => v.classList.toggle('active', v.id === id)); window.scrollTo({top:0, behavior:'smooth'}); if (id === 'cards') renderCard(); if (id === 'quiz') renderQuiz(); if (id === 'speak') renderSpeak(); if (id === 'pair') renderPair(); if (id === 'today') renderToday(); if (id === 'categorySelect') renderCategorySelect(); }
...
function startCategory(id){const g=genreData.find(x=>x.id===id); activeGenreId=id; cards=buildCards(g); cardIndex=quizIndex=speakIndex=pairIndex=0; quizCorrect.clear(); speakCorrect.clear(); pairCorrect.clear(); $('#cards h2').textContent=`${g.name}の ことば`; showView(pendingMode);}
function showClear(message, nextMode){$('#clearTitle').textContent=`${genreData.find(g=>g.id===activeGenreId)?.name||''}カテゴリ クリア！`; $('#clearMessage').textContent=message; $('#clearContinue').textContent=nextMode==='quiz'?'問題に ちょうせん　›':'ホームへ　›'; $('#clearContinue').dataset.nextMode=nextMode||'home'; showView('clear');}
$('#clearContinue').addEventListener('click',()=>{const mode=$('#clearContinue').dataset.nextMode; if(mode==='home')showView('home'); else showCategorySelect(mode);});
function renderCard() {
...
$('#nextCard').addEventListener('click', () => { if(cardIndex===cards.length-1){completeMode('treasure'); showClear('たからばこクリア！ れんしゅうがアンロックされたよ。','quiz'); return;} cardIndex++; renderCard(); });
function renderQuiz() {
...
function answer(btn, correct) { document.querySelectorAll('.choice').forEach(b => b.disabled = true); if (btn.dataset.answer === correct) { quizCorrect.add(quizIndex); btn.classList.add('correct'); $('#result').textContent = 'せいかい！ すごいね 🎉'; $('#result').classList.add('ok'); } else { btn.classList.add('wrong'); $('#result').textContent = `おしい！ せいかいは「${correct}」だよ。`; $('#result').classList.add('ng'); document.querySelector(`[data-answer="${correct}"]`).classList.add('correct'); } $('#nextQuiz').disabled = false; }
$('#nextQuiz').addEventListener('click', () => { if(quizIndex===cards.length-1){if(quizCorrect.size===cards.length){completeMode('quiz'); showClear('このジャンルの問題を ぜんぶ正解したよ！','home'); return;} quizCorrect.clear(); quizIndex=0; renderQuiz(); return;} quizIndex++; renderQuiz(); });
function renderSpeak(){const c=cards[speakIndex]; $('#speakImage').src=c.img; $('#speakImage').alt=c.en+'のイラスト'; $('#speakEn').textContent=c.en; $('#speakCount').textContent=`${speakIndex+1} / ${cards.length}`; $('#speakResult').textContent='まずは おてほんを きいてみよう。'; $('#speakResult').className='result'; $('#nextSpeak').disabled=!speakCorrect.has(speakIndex);}
...
$('#nextSpeak').addEventListener('click',()=>{if(!speakCorrect.has(speakIndex))return; if(speakIndex===cards.length-1){if(speakCorrect.size===cards.length){completeMode('speak'); showClear('まねっこスピークを 3つ クリアしたよ！','home'); return;} speakIndex=0;} else speakIndex++; renderSpeak();});
let speakRecorder, speakChunks=[];
$('#speakPractice').addEventListener('click',async()=>{const btn=$('#speakPractice'), status=$('#speakResult'); if(speakRecorder?.state==='recording'){speakRecorder.stop();return;} try{const stream=await navigator.mediaDevices.getUserMedia({audio:true}); speakChunks=[]; speakRecorder=new MediaRecorder(stream); speakRecorder.ondataavailable=e=>speakChunks.push(e.data); speakRecorder.onstop=async()=>{stream.getTracks().forEach(t=>t.stop()); btn.textContent='🎙️ いってみる'; status.textContent='はつおんを きいているよ…'; try{const c=cards[speakIndex]; const r=await fetch(`/api/pronounce?expected=${encodeURIComponent(c.en)}`,{method:'POST',headers:{'Content-Type':speakRecorder.mimeType||'audio/webm'},body:new Blob(speakChunks,{type:speakRecorder.mimeType||'audio/webm'})}); const d=await r.json(); status.textContent=d.feedback||'もういちど やってみよう。'; status.className='result '+(d.matched?'ok':'ng'); if(d.matched){speakCorrect.add(speakIndex); $('#nextSpeak').disabled=false;} else {$('#nextSpeak').disabled=true; status.textContent+=' もういちど ためしてみよう。';}}catch{status.textContent='うまく ききとれなかったよ。';}}; speakRecorder.start(); btn.textContent='⏹ おわる'; status.textContent='いってみよう！'; setTimeout(()=>{if(speakRecorder?.state==='recording')speakRecorder.stop()},2500);}catch{status.textContent='マイクが つかえないようです。';}});
function renderPair(){const c=cards[pairIndex]; $('#pairWord').textContent=c.en; $('#pairCount').textContent=`${pairIndex+1} / ${cards.length}`; $('#pairResult').textContent=''; $('#pairResult').className='result'; $('#nextPair').disabled=true; const opts=[c,...cards.filter((_,i)=>i!==pairIndex)].sort(()=>Math.random()-.5); $('#pairChoices').innerHTML=opts.map(x=>`<button class="choice image-choice" data-answer="${x.en}"><img src="${x.img}" alt="${x.jp}"><span>${x.jp}</span></button>`).join(''); document.querySelectorAll('#pairChoices .choice').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('#pairChoices .choice').forEach(x=>x.disabled=true); const ok=b.dataset.answer===c.en; if(ok) pairCorrect.add(pairIndex); b.classList.add(ok?'correct':'wrong'); if(!ok) document.querySelector(`#pairChoices [data-answer="${c.en}"]`).classList.add('correct'); $('#pairResult').textContent=ok?'せいかい！ ぴったりペアだね 🎉':`おしい！ せいかいの 絵は「${c.jp}」だよ。もう一周してみよう。`; $('#pairResult').classList.add(ok?'ok':'ng'); $('#nextPair').disabled=false;}));}
$('#nextPair').addEventListener('click',()=>{if(pairIndex===cards.length-1){if(pairCorrect.size===cards.length){completeMode('pair'); showClear('えとことばペアを 3つ ぜんぶ正解したよ！','home'); return;} pairCorrect.clear(); pairIndex=0; renderPair(); return;} pairIndex++; renderPair();});
let analysisIndex=0, analysisRecorder, analysisChunks=[];
const analysisWords=[{en:'cat',sounds:'k æ t'},{en:'dog',sounds:'d ɔ g'},{en:'apple',sounds:'æ p əl'}];
function renderAnalysis(){const w=analysisWords[analysisIndex]; $('#analysisWord').textContent=w.en; $('#expectedSounds').textContent=w.sounds; document.querySelectorAll('.analysis-tabs span').forEach((x,i)=>x.classList.toggle('active',i===analysisIndex));}
document.querySelectorAll('.analysis-tabs span').forEach((b,i)=>b.addEventListener('click',()=>{analysisIndex=i;renderAnalysis();}));
$('#analysisSample').addEventListener('click',async()=>{const w=analysisWords[analysisIndex];try{const r=await fetch('/api/speak',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:w.en,target:w.en})});await new Audio(URL.createObjectURL(await r.blob())).play();}catch{}});
$('#analysisRecord').addEventListener('click',async()=>{if(analysisRecorder?.state==='recording'){analysisRecorder.stop();return;}try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});analysisChunks=[];analysisRecorder=new MediaRecorder(stream);analysisRecorder.ondataavailable=e=>analysisChunks.push(e.data);analysisRecorder.onstop=async()=>{stream.getTracks().forEach(t=>t.stop());$('#analysisRecord').textContent='🎙️ 録音して判定';$('#analysisStatus').textContent='分析中…';try{const expected=analysisWords[analysisIndex].en;const r=await fetch(`/api/pronounce?expected=${expected}`,{method:'POST',headers:{'Content-Type':analysisRecorder.mimeType||'audio/webm'},body:new Blob(analysisChunks,{type:analysisRecorder.mimeType||'audio/webm'})});const d=await r.json();$('#analysisScore').textContent=Number.isFinite(Number(d.score))?Math.round(Number(d.score)):'--';$('#analysisSummary').textContent=d.matched?'いい発音です。細かい音も確認できます。':'もう少し練習してみましょう。';$('#heardSounds').textContent=d.transcript||'（聞き取れませんでした）';const errors=d.differences?.errors||[];$('#analysisErrors').textContent=d.analysis==='openpronounce'?(errors.length?errors.slice(0,3).map(e=>`${e.word||expected}: ${e.expected||'--'} → ${e.actual||'（なし）'}`).join(' / '):'音素エラーは見つかりませんでした。'):'音素比較はOpenPronounce接続時に表示します。';$('#analysisStatus').textContent=(d.analysis==='openpronounce'?'OpenPronounce分析：':'簡易分析：')+(d.feedback||'講評を確認してください。');}catch{$('#analysisStatus').textContent='分析できませんでした。もう一度試してください。';}};analysisRecorder.start();$('#analysisRecord').textContent='⏹ 録音をとめる';$('#analysisStatus').textContent='録音中…';setTimeout(()=>{if(analysisRecorder?.state==='recording')analysisRecorder.stop()},2500);}catch{$('#analysisStatus').textContent='マイクが使えません。';}});
renderAnalysis();
