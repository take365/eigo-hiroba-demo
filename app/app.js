...
let cardIndex = 0, quizIndex = 0, speakIndex = 0, pairIndex = 0, quizCorrect = new Set(), speakCorrect = new Set();
...
function startCategory(id){const g=genreData.find(x=>x.id===id); activeGenreId=id; cards=buildCards(g); cardIndex=quizIndex=speakIndex=pairIndex=0; quizCorrect.clear(); speakCorrect.clear(); $('#cards h2').textContent=`${g.name}の ことば`; showView(pendingMode);}
...
$('#nextPair').addEventListener('click',()=>{if(pairIndex===cards.length-1){completeMode('pair'); showClear('えとことばペアを 3つ クリアしたよ！','home'); return;} pairIndex++; renderPair();});
